...
class _FarmerOrdersScreenState extends State<FarmerOrdersScreen> {
            ...
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  return const Center(
                    child: Text("Error: Check Firebase Index"),
                  );
                }

                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                  return const Center(
                    child: Text("No orders found for your farm."),
                  );
                }
                ...
                if (docs.isEmpty) {
                  return const Center(
                    child: Text("No orders matching this status."),
                  );
                }
  ...
  Widget _buildManagementCard(
    ...
    String address =
        data['deliveryAddress'] ?? data['address'] ?? 'No address provided';
        ...
        try {
          ...
          if (mounted) {
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(SnackBar(content: Text("Order marked as $label")));
          }
        } catch (e) {
