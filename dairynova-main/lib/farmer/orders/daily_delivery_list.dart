...
class DailyDeliveryList extends StatelessWidget {
  ...
  bool _isCurrentlyPaused(Map<String, dynamic> data) {
    if (data['pauseStartDate'] == null || data['pauseEndDate'] == null) {
      return false;
    }
  ...
  Future<void> _updateDeliveryStatus(
    ...
    if (newStatus == 'Delivered') {
      ...
      DateTime currentNextDate = (data['nextDeliveryDate'] as Timestamp)
          .toDate();
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          final activeSubs = allDocs.where((doc) {
            ...
            bool isDueToday =
                next != null && next.toDate().isBefore(endOfToday);
            bool wasDoneToday =
                last != null &&
                _normalize(last.toDate()).isAtSameMomentAs(startOfToday);

            return isDueToday || wasDoneToday;
          }).toList();

          if (activeSubs.isEmpty) {
            return const Center(child: Text("No deliveries for today."));
          }
  ...
  Widget _buildSummaryHeader(
    ...
    for (var doc in docs) {
      ...
      bool isDoneToday =
          ...
          _normalize(
            last.toDate(),
          ).isAtSameMomentAs(_normalize(DateTime.now()));
  ...
  Widget _buildDeliveryTile(
    ...
    bool isDoneToday =
        last != null &&
        _normalize(last.toDate()).isAtSameMomentAs(_normalize(DateTime.now()));
          ...
          ListTile(
            leading: CircleAvatar(
              backgroundColor: isDoneToday
                  ? Colors.green
                  : (isPaused ? Colors.grey : _getStatusColor(status)),
              child: Icon(
                isDoneToday
                    ? Icons.check
                    : (isPaused ? Icons.pause : Icons.delivery_dining),
                color: Colors.white,
              ),
            ),
            title: Text(
  ...
  void _showHistoryCalendar(BuildContext context, Map<String, dynamic> data) {
    ...
    Set<DateTime> deliveredDates = history
        .map((e) => _normalize((e as Timestamp).toDate()))
        .toSet();

    DateTime? pauseStart = data['pauseStartDate'] != null
        ? _normalize(data['pauseStartDate'].toDate())
        : null;
    DateTime? pauseEnd = data['pauseEndDate'] != null
        ? _normalize(data['pauseEndDate'].toDate())
        : null;
            ...
            children: [
              Expanded(
                child: TableCalendar(
                  ...
                  eventLoader: (day) => deliveredDates.contains(_normalize(day))
                      ? ['Delivered']
                      : [],
                  ...
                  calendarBuilders: CalendarBuilders(
                    defaultBuilder: (context, day, focusedDay) {
                      ...
                      if (pauseStart != null && pauseEnd != null) {
                        if ((normalizedDay.isAtSameMomentAs(pauseStart) ||
                                normalizedDay.isAfter(pauseStart)) &&
                            (normalizedDay.isAtSameMomentAs(pauseEnd) ||
                                normalizedDay.isBefore(pauseEnd))) {
                          return Container(
              ...
              const Row(
                ...
                children: [
                  ...
                  Text(" Paused Dates", style: TextStyle(fontSize: 10)),
                ],
              ),
            ],
          ),
