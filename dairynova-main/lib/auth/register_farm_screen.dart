...
class _RegisterFarmScreenState extends State<RegisterFarmScreen> {
  ...
  XFile? _newCnicFile;
  final List<XFile?> _newFarmFiles = List.filled(5, null);
  ...
  void initState() {
    ...
    _ownerController = TextEditingController(
      text: widget.existingFarm?.owner ?? "",
    );
    _locationController = TextEditingController(
      text: widget.existingFarm?.location ?? "",
    );
  }
  ...
  Future<String?> _uploadImage(XFile imageFile) async {
    try {
      ...
      request.files.add(
        http.MultipartFile.fromBytes('image', data, filename: imageFile.name),
      );
  ...
  Future<void> _submitData() async {
    ...
    bool hasCnic =
        _newCnicFile != null ||
        (widget.existingFarm?.cnicUrl.isNotEmpty ?? false);
    if (!hasCnic) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("CNIC photo is required")));
      return;
    }
    ...
    try {
      ...
      batch.update(userRef, {'status': 'pending'});
      ...
      if (mounted) {
        Navigator.pushAndRemoveUntil(
          ...
          MaterialPageRoute(
            builder: (context) =>
                FarmStatusScreen(farmName: _nameController.text),
          ),
          (route) => false,
        );
      }
  ...
  Widget _imageBox(int index, bool isCnic) {
    bool isFlagged =
        !isCnic &&
        (widget.existingFarm?.flaggedImages.contains(index) ?? false);
    String? existingUrl = isCnic
        ? widget.existingFarm?.cnicUrl
        : widget.existingFarm?.farmPhotos[index];
    ...
    return InkWell(
      ...
      child: Container(
        decoration: BoxDecoration(
          border: Border.all(
            color: isFlagged ? Colors.red : Colors.grey.shade400,
            width: isFlagged ? 2.5 : 1,
          ),
