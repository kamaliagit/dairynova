...
import 'package:flutter/foundation.dart'
    show kIsWeb; // Essential to prevent web crashes
...
class BillingSummaryScreen extends StatelessWidget {
  ...
  Future<String> _getCustomerName(String uid) async {
    ...
    return userDoc.exists
        ? (userDoc.data() as Map<String, dynamic>)['name'] ?? "Customer"
        : "Customer";
  }
  ...
  Future<String> _getFarmName(String farmId) async {
    ...
    return farmDoc.exists
        ? (farmDoc.data() as Map<String, dynamic>)['farmName'] ?? "Farm"
        : "Farm";
  }
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          List<QueryDocumentSnapshot> currentMonthOrders = allDocs.where((doc) {
            ...
            Timestamp? dateStamp =
                data['orderDate'] ?? data['nextDeliveryDate'];
              ...
              children: [
                ...
                ...currentMonthOrders.map((doc) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      title: Text(
                        "${data['farmName'] ?? 'Farm'} - #$shortOrderId",
                      ),
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    onPressed: currentMonthOrders.isEmpty
                        ? null
                        : () async {
  ...
  Future<void> _generateInvoice(
    ...
    List<List<String>> tableData = [
      <String>['Date', 'Order ID', 'Farm', 'Items & Qty', 'Amount'],
    ];

    for (var doc in orders) {
      final data = doc.data() as Map<String, dynamic>;
      Timestamp ts =
          data['orderDate'] ?? data['nextDeliveryDate'] ?? Timestamp.now();
      List items = data['items'] ?? [];
      ...
      String farmName =
          data['farmName'] ?? await _getFarmName(data['farmId'] ?? "");
      String shortOrderId = doc.id.substring(0, 6).toUpperCase();
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.Text(
                "Billing Period: ${DateFormat('MMMM yyyy').format(DateTime.now())}",
              ),
              pw.SizedBox(height: 20),
              ...
              pw.Align(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text(
                      "Total Payable: Rs. ${total.toStringAsFixed(0)}",
                      style: pw.TextStyle(
                        fontSize: 20,
                        fontWeight: pw.FontWeight.bold,
                        color: PdfColors.green900,
    ...
    if (!kIsWeb) {
      try {
        ...
        final String fileName =
            "Invoice_${DateFormat('MMM_yyyy').format(DateTime.now())}.pdf";
