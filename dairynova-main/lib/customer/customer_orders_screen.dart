      ...
      body: user == null
          ...
          : StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                /// Sort newest first
                docs.sort((a, b) {
                  Timestamp? aDate = a.data().toString().contains('orderDate')
                      ? a['orderDate']
                      : null;
                  Timestamp? bDate = b.data().toString().contains('orderDate')
                      ? b['orderDate']
                      : null;
  ...
  Widget _buildOrderCard(
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text(
          "Rs. ${totalAmount.toStringAsFixed(0)} | $status",
          style: TextStyle(
  ...
  void _confirmCancel(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          "Are you sure? Items will be returned to the farm's stock.",
        ),
        actions: [
          TextButton(
              ...
              try {
                ...
                await firestore.runTransaction((transaction) async {
                  ...
                  for (var item in items) {
                    String? productId =
                        item['productId']; // Ensure you save productId in CartProvider
                    if (productId != null) {
                      DocumentReference productRef = firestore
                          .collection('products')
                          .doc(productId);
                      DocumentSnapshot productSnap = await transaction.get(
                        productRef,
                      );
