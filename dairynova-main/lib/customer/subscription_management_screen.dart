...
class SubscriptionManagementScreen extends StatelessWidget {
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => const BillingSummaryScreen(),
              ),
            ),
          ),
        ],
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (subData['nextDeliveryDate'] != null) {
                nextDate = DateFormat(
                  'dd MMM yyyy',
                ).format((subData['nextDeliveryDate'] as Timestamp).toDate());
              }

              double total =
                  double.tryParse(subData['totalAmount'].toString()) ?? 0.0;
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              IconButton(
                                ...
                                onPressed: () => _showCustomerHistoryCalendar(
                                  context,
                                  subData,
                                ),
                              ),
                              ...
                              children: [
                                if (status == 'Paused')
                                  ...
                                  Expanded(
                                    child: OutlinedButton.icon(
                                      onPressed: () =>
                                          _showPausePicker(context, subId),
                                      icon: const Icon(
  ...
  void _showCustomerHistoryCalendar(
    ...
    DateTime? pauseStart = data['pauseStartDate'] != null
        ? DateTime(
            data['pauseStartDate'].toDate().year,
            data['pauseStartDate'].toDate().month,
            data['pauseStartDate'].toDate().day,
          )
        : null;
    DateTime? pauseEnd = data['pauseEndDate'] != null
        ? DateTime(
            data['pauseEndDate'].toDate().year,
            data['pauseEndDate'].toDate().month,
            data['pauseEndDate'].toDate().day,
          )
        : null;
    ...
    if (now.hour >= 22) {
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text(
            "Cancellations after 10 PM apply after tomorrow's delivery.",
          ),
          actions: [
            TextButton(
